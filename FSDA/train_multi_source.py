...
if __name__ == '__main__':
    ...
    model_file_d1 = './logs/train_source/best_model_d2.pth.tar'
    model_file_d2 = './logs/train_source/best_model_d1.pth.tar'
    ...
    print('==> Loading %s model file: %s' %
          (model_t1.__class__.__name__, model_file_d1))
    ...
    print('==> Loading %s model file: %s' %
          (model_t2.__class__.__name__, model_file_d2))
    ...
    print('==> Loading %s model file: %s' %
          (model_student.__class__.__name__, model_file))
    ...
    iter_num = 0
    for epoch_num in tqdm.tqdm(range(50), ncols=70):
        ...
        for batch_idx, (sample) in enumerate(train_loader):
            ...
            #
            pred_t1, _, _, _ = model_t1(data_noise)
            pred_t2, _, _, _ = model_t2(data_noise)
            # #
            loss_consis_1 = softmax_mse_loss(pred_stu, pred_t1)
            loss_consis_2 = softmax_mse_loss(pred_stu, pred_t2)
            loss = loss_seg.mean() + torch.mean(loss_consis_1 + loss_consis_2)
            loss.backward()

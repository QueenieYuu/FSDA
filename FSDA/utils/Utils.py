...
def colorize_mask(mask):
    ...
    mask_cup = construct_color_img(mask_cup)
    mask_disc = construct_color_img(mask_disc)
    ...
    mask_cup = Image.fromarray(mask_cup.astype(np.uint8)).convert('P')
    mask_cup.putpalette(palette)
    mask_cup = mask_cup.convert('RGBA')
    ...
    mask_disc = Image.fromarray(mask_disc.astype(np.uint8)).convert('P')
    mask_disc.putpalette(palette)
    mask_disc = mask_disc.convert('RGBA')

    mask = Image.blend(mask_disc, mask_cup, 0.5)


    # mask_temp = mask[0]
    # mask_cup = mask_temp[0].permute(1, 2, 0)
    # mask_cup = mask_cup.cpu().detach().numpy()
    # mask_cup = Image.fromarray(mask_cup.astype(np.uint8)).convert('P')
    #mask_cup.putpalette(palette)
    #
    # palette = [255,255,255,128,128,128]
    # zero_pad = 256 * 3 - len(palette)
    #
    # for i in range(zero_pad):
    #     palette.append(0)
    #
    # mask_disc = mask_temp[1].permute(1, 2, 0)
    # mask_disc = mask_disc.cpu().detach().numpy()
    # mask_disc = Image.fromarray(mask_disc.astype(np.uint8)).convert('P')
    # mask_disc.putpalette(palette)
    #
    return mask_cup, mask_disc, mask

def compute_prototype(feature, mask, cls = 0):
    b,c,h,w = mask.size()
    slice_idx = [[i] for i in range(b)]
    mask = mask[slice_idx, cls]
    mask = F.interpolate(mask, size=feature.size()[2:], mode='bilinear', align_corners=True)
    prototype = (mask > 0)* feature
    prototype = torch.mean(torch.mean(prototype,2),2)
    return prototype
...
def construct_color_img(prob_per_slice):
    ...
    #im_color = cv2.applyColorMap(img, cv2.COLORMAP_BONE)
    return img

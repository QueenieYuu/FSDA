...
def to_multilabel(pre_mask, classes = 5):
    mask = np.zeros((pre_mask.shape[0], pre_mask.shape[1], classes))

    for i, unique_value in enumerate(np.unique(pre_mask)):
        mask[:, :, i][pre_mask == unique_value] = 1
...
class Normalize_tf(object):
    ...
    def __call__(self, sample):
        ...
        # img_s = Image.fromarray(np.uint8(img)).convert('RGB')
        # img_s.save('0.png')
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        # map = transform(sample['map'])
        # boundary = transform(sample['boundary'])

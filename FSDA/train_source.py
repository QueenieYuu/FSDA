...
def main():
    ...
    domain_source = DL.FundusSegmentation(base_dir=args.data_dir,dataset=args.datasetS, split='train/ROIs', transform=composed_transforms_tr)
    domain_loaderS = DataLoader(domain_source, batch_size=args.batch_size, shuffle=True, num_workers=2, pin_memory=True,drop_last=True)
    ...
    domain_loaderT = DataLoader(domain_T, batch_size=args.batch_size, shuffle=False, num_workers=2, pin_memory=True,drop_last=True)
    ...
    domain_loader_val = DataLoader(domain_val, batch_size=args.batch_size, shuffle=False, num_workers=2, pin_memory=True,drop_last=True)

    # dataroot_train = os.getcwd() + '/' + args.data_dir +'/' + args.datasetS + '/train/ROIs/image'
    # dataroot_test = os.getcwd() + '/' + args.data_dir + '/' + args.datasetS + '/test/ROIs/image'
    #
    # composed_transforms_tr.transforms.insert(0, Augmentation(aug, dataroot_train))
    # composed_transforms_ts.transforms.insert(0, Augmentation(aug, dataroot_test))
    #
    # domain_cyclegan = DL.FundusSegmentation(base_dir=args.data_dir,dataset=args.datasetS, split='train/ROIs', transform=composed_transforms_tr, set='cyclegan')
    # domain_loaderS_cyclegan = DataLoader(domain_cyclegan, batch_size=args.batch_size, shuffle=False, num_workers=2, pin_memory=True,drop_last=True)
    # #
    domain_fda = DL.FundusSegmentation(base_dir=args.data_dir,dataset=args.datasetS, split='train/ROIs',transform=composed_transforms_tr, set = 'fda')
    domain_loaderS_fda = DataLoader(domain_fda, batch_size=args.batch_size, shuffle=False, num_workers=2, pin_memory=True,drop_last=True)
    ...
    trainer = Trainer.Trainer(
        ...
        lr_decrease_rate=args.lr_decrease_rate,
        val_loader=domain_loader_val,
        domain_loaderS_1=domain_loaderS_fda,
        domain_loaderS_2=domain_loaderS,
        # domain_loaderS_3=domain_loaderS_fda,
        domain_loaderT=domain_loaderT,
        out=args.out,
        max_epoch=args.max_epoch,
        stop_epoch=args.stop_epoch,
